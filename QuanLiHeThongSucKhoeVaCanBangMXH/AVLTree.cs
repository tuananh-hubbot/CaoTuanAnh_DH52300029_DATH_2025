...
namespace QuanLiHeThongSucKhoeVaCanBangMXH
{
    internal class AVLTree
    {
        ...
        public void PostOrder(Node node, List<NguoiDung> list)
        {
            if (node != null)
            {
        ...
        public void LevelOrder(Node root, List<NguoiDung> list)
        {
            if (root == null) return;
        ...
        private int tinhChieuCao(Node node)
        {
            if(node == null) return 0;
            return 1+ Math.Max(tinhChieuCao(node.Right),tinhChieuCao(node.Left));
        }
        ...
        public double tongNodeLa(Node node, Func<NguoiDung,double> selector)
        {
            if (node == null) return 0;
            if (node.Left == null && node.Right == null) return selector(node.Data);
            return tongNodeLa(node.Left, selector) + tongNodeLa(node.Right, selector);
        }
        public double tongNode(Node node, Func<NguoiDung,double> selector)
        {
            if(node == null) return 0;
